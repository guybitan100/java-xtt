...
public class StringUtils {
	...
	public static byte[] stringToBytes(String s) {
		...
		for (int i = 0; i < toReturn.length; i++) {
			toReturn[i] = (byte) Integer.parseInt(s.substring(i * 2, i * 2 + 2), 16);
		}
		return toReturn;
	}
		...
		for (int i = 0; i < readBytes; i++) {
			{
				ind = bytes[i];
				if (ind <0) ind = ind*-1;
				ipData.append(ind +  ( i<readBytes-1 ? "." : "") );
			}
		}
	...
	public static String hexToIp(String s) 
	{
		...
		{
			if(i<3)
				toReturn = toReturn +  Integer.parseInt(s.substring(i * 2, i * 2 + 2), 16) + ".";
			else
				toReturn = toReturn +  Integer.parseInt(s.substring(i * 2, i * 2 + 2), 16);
		}
		return toReturn;
	}
	public static String hexStringToIp(String hexStr) {
		
		return bytesToIpString(stringToBytes(hexStr));
	}
	/**
