        ...
        try
        {
            ...
            switch( kind )
            {
                ...
                case TCKind._tk_objref:
                    XTTProperties.printInfo(this.getClass().getName()+": "+"Objectref.");
                    TestServer s = TestServerHelper.narrow(a.extract_Object());
                    Any any = org.omg.CORBA.ORB.init().create_any();
                    any.insert_string("hallo");
                    result = s.generic( any );
                    break;
                case TCKind._tk_any:
                    XTTProperties.printInfo(this.getClass().getName()+": "+"Test.");
                    Any inner_any = a.extract_any();
                    result = generic( inner_any );
                    break;
                case TCKind._tk_alias:
                    XTTProperties.printInfo(this.getClass().getName()+": "+"Alias: ");
                    Any alias_any = a;
                    ...
                    {
                        XTTProperties.printInfo(this.getClass().getName()+": "+"Unknown alias, type kind: " +
                                           alias_any.type().kind().value() );
                    }
                    break;
                default:
                    XTTProperties.printInfo(this.getClass().getName()+": "+"Unknown, kind " + a.type().kind().value());
            }
        } catch ( Exception e )
        {
            e.printStackTrace();
        }
        XTTProperties.printInfo(this.getClass().getName()+": Returning result: "+result);
        return result;
    }
}
